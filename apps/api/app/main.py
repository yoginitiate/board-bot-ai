...
from board_bot.llm.factory import create_chat_llm
...
repo = PostgresRepo(cfg.get("postgres", "dsn", default="postgresql+psycopg://user:password@db-host:5432/boardbot"))
glossary_service = GlossaryService(
    repo.engine,
    refresh_enabled=bool(cfg.get("glossary", "refresh", "enabled", default=False)),
    ttl_seconds=int(cfg.get("glossary", "refresh", "ttl_seconds", default=600)),
)
try:
    glossary_service.load_cache()
except Exception:
    pass
mcp = MCPClient(cfg.get("mcp", "url", default="http://mcp:9000"), timeout=float(cfg.get("mcp", "timeout_s", default=8)))
...
if api_key:
    llm = create_chat_llm(provider=provider, model=model_name, api_key=api_key)
...
graph = compile_graph(llm=llm, tool_call=tool_call, policy_map=policies, logger=logger, repo=repo, glossary=glossary_service)
...
def process_case(req: ProcessRequest) -> dict[str, Any]:
    ...
    state = {
        ...
        "glossary": {"terms": [], "normalized_terms_used": False},
        "config": cfg.data,
    }
