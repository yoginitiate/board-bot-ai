...
def plan_engine(state: AgentState, tool_call) -> AgentState:
    ...
    for idx, step in enumerate(state["plan"]["steps"]):
        ...
        if isinstance(res, dict) and res.get("ok") is False:
            ok = False
            err = str(res.get("reason", "tool_failed"))
            state.setdefault("decision", {}).update({"type": "DRAFT", "reason": "tool_failure"})
...
def scenario_subgraph(state: AgentState) -> AgentState:
    ...
    def render(issue):
        ...
        gcfg = state.get("config", {}).get("glossary", {})
        if bool(gcfg.get("expose_to_customer", False)):
            terms = state.get("glossary", {}).get("terms", [])
            if terms:
                body += "\n\n[용어 설명]"
                for t in terms:
                    body += f"\n- {t.get('term','')}: {(t.get('description','') or '')[:int(gcfg.get('description_max_chars',120))]}"
...
def agent_generate_subgraph(state: AgentState, llm, tool_call) -> AgentState:
    ...
    if llm is None:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "너는 게시판 상담 에이전트다. 필요한 도구를 호출해 답변을 만든다. 용어 정의를 참고해 오해 없이 답하되 고객에게 정의를 그대로 복붙하지 말고 문맥에 맞게 간단히 설명하라."),
            ("human", "{input}"),
...
def validate_grounding(state: AgentState) -> AgentState:
    ...
    glossary_terms = state.get("glossary", {}).get("terms", [])
    glossary_hit = any((t.get("term") or "") in answer for t in glossary_terms)
    glossary_mismatch = 0
    for t in glossary_terms:
        term = t.get("term", "")
        desc = t.get("description", "")
        if term and term in answer:
            # soft rule: 용어를 썼는데 설명 핵심 단어가 하나도 없으면 맥락 불일치로 간주
            desc_tokens = [w for w in re.findall(r"[0-9A-Za-z가-힣_]+", desc) if len(w) >= 2][:6]
            if desc_tokens and not any(tok in answer for tok in desc_tokens):
                glossary_mismatch += 1
    glossary_penalty = min(glossary_mismatch * 0.05, 0.2)
    score = 0.5 * issue_score + 0.3 * citation_score + 0.2 * policy_score - glossary_penalty
    ...
    emit_metric(state, "validator_result", tags={"phase": "GROUNDING", "passed": passed, "fail_reason_code": "" if passed else "GROUNDING_LOW_SCORE", "success": True, "grounding_score": score, "glossary_hit": glossary_hit, "term_count": len(glossary_terms)})
    return state
