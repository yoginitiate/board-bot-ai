"""Master/Executor 통합 LangGraph 파이프라인."""
...
def _with_node_logging(node_name: str, fn: Callable[[AgentState], AgentState]) -> Callable[[AgentState], AgentState]:
    def _wrapped(state: AgentState) -> AgentState:
        ...
        try:
            ...
            emit_metric(state, "node_execution", tags={"node": node_name, "latency_ms": latency_ms, "success": ok, "error_code": err[:64], "masked_len": masked_len})

    return _wrapped


def compile_graph(llm, tool_call, policy_map, logger, repo):
