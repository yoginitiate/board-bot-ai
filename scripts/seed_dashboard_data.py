...
DSN = os.getenv("POSTGRES_DSN", "postgresql+psycopg://user:password@db-host:5432/boardbot")
...
def ins_metric(conn, case_id: str, name: str, tags: dict, value: float = 1.0, created_at: datetime | None = None):
    conn.execute(
        text("INSERT INTO board_bot.t_metrics_events(case_id, metric_name, value, tags_jsonb, created_at) VALUES (:c,:n,:v,CAST(:t AS jsonb),:d)"),
...
with engine.begin() as conn:
    for i in range(1, 201):
        ...
        conn.execute(
            text("INSERT INTO board_bot.t_cases(case_id, thread_id, created_at, updated_at, state_jsonb) VALUES (:c,:t,:d,:d,CAST(:s AS jsonb)) ON CONFLICT (case_id) DO NOTHING"),
            {
                "c": case_id,
                "t": case_id,
                "d": created,
